...
/**
 * Session task class to execute CALCULATE_CONFLICTS session
 *
 * @author: junsu.shin@cnhind.com
 */
public class CalculateConflictsTask extends SessionOperationTask<Void> {
   private static final Logger logger = LoggerFactory.getLogger(CalculateConflictsTask.class);

   public CalculateConflictsTask(@Nonnull Mediator mediator, @Nonnull SessionNotifier notifier) {
   ...
   protected void execute(@Nonnull Session session) throws SessionException {
      ...
      try {
         ...
         logger.debug("Calculate conflicts to address: {}", SessionUtil.addressToString(addresses));
         if (addresses != null && addresses.length > 0) {
            List<Operation> operations = getMediator().calculateConflicts(session.getOperations(), addresses);
