...
/**
 * Session task class to execute PASTE session
 *
 */
public class PasteTask extends SessionOperationTask<Void> {
   private static final Logger logger = LoggerFactory.getLogger(PasteTask.class);

   public PasteTask(@Nonnull Mediator mediator, @Nonnull SessionNotifier notifier) {
   ...
   protected void processSession(@Nonnull Session session) throws SessionException {
      ...
      try {
         session.setResults(getMediator().pasteOperations(session.getOperations(), null));
         ...
         for (Rsp<Process> ret : session.getResults()) {
            ...
            if (ret.wasReceived() && ret.getValue() != null && ret.getValue().getResult() != null) {
               ...
               throw new SessionException(ErrorCode.PASTE_ERROR);
            }
         }
         ...
         logger.error("Send exception in PASTE: ", e);
         session.setResultCode(Process.Result.ERROR);
         throw new SessionException(ErrorCode.PASTE_ERROR);
      }
   }
